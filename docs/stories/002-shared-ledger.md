...
superseded by ticket 028), so step 7 drives the shipped
`/memberships/acceptances` route directly; the accept surface it asserts is the
document that screen will render.
...
8. Read the membership state: `dc exec -T alice mabel membership list --ledger
   mabel-demo-co`.
9. The warning case, on a raw root instead of an identity root. Alice invites
   bob as a controller of her own person ledger, bob's wallet answers the
   surface, and alice cancels the open invitation:
   ```sh
   ...
   curl -fsS -X POST \
     -H 'Origin: http://127.0.0.1:9082' -H 'Content-Type: application/json' \
     --data "{\"invitation_bundle_base64\":\"$(base64 -w0 /tmp/raw.bundle)\"}" \
     "http://127.0.0.1:9082/api/identities/$bob_id/memberships/acceptances"
   dc exec -T alice mabel membership remove --ledger alice --by alice \
     --member "$bob_id"
   ```
10. Back in alice's UI on the `org_id` page, paste `bob_id` into
...
- Step 9's surface, for the same invitee on a raw-rooted ledger, answers `root:
  "raw"`, `controller_on_raw_root: true` and `warning` exactly:
  `accepting a controller role on a raw-rooted ledger means signing as
  <alice_id>: every event you append to it is that identity's own event`.
- Step 9's removal prints `removed <bob_id> at seq N of <alice_id>` and
  `cancelled open invitation <invitation event>`.
...
- Step 11 exits 30 with `details.reason == "all_witnesses_failed"`,
  ...
  names it.
- Step 12's push report reads `push-status-<witness_id>` `accepted` and
  `push-stored-<witness_id>` `5`.
- Step 13 exits 0 and prints `trusted: true`, then `valid as of seq 4 of
