...
These three run on every push regardless of signing. With no Apple secrets set
the macOS artifact is unsigned, and Gatekeeper refuses an unsigned download; the
workflow logs a notice saying so and naming the secrets it did not find. The
signed builds are described in [Signing](#signing) below.
...
Signing is off until the secrets exist. The workflow reads them, decides in a
...
Nothing is partial. The macOS job wants six of them, `APPLE_CERTIFICATE`,
`APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, `APPLE_API_ISSUER`,
`APPLE_API_KEY` and `APPLE_API_KEY_CONTENT`, and with any one of them missing it
builds unsigned. There is no signed but un-notarized middle state on purpose:
Gatekeeper refuses that download exactly as it refuses an unsigned one.

With all six present, `cargo tauri build` does the work itself. Tauri imports the
`.p12` into a keychain it creates and deletes on its own, signs the `.app` with
the identity under the hardened runtime, submits it to notarytool with the API
key, and staples the ticket. The workflow adds two steps around that: it
notarizes and staples the `.dmg`, which tauri signs but does not notarize, and it
runs `codesign --verify` and `stapler validate` over both so a run that produced
an unusable bundle fails rather than uploads.

`APPLE_API_KEY_CONTENT` exists because tauri has no variable for the key body. It
only reads `APPLE_API_KEY_PATH`, so the workflow writes the key to a file under
the runner's temp directory, outside the workspace where no artifact can pick it
up, and deletes it at the end of the job.
...
what is checked here is that the workflow parses, that actionlint and yamllint
pass, that every shell script passes shellcheck, and that the variable names
...
- Signing waits on the secrets in [Signing](#signing). Until they are set both
  builds are unsigned and are for developers and testers rather than for users.
